...
fn load_resource<'d, 's, D, L, P>(
    locale: &L,
    resource_key: ResourceKey,
    destination: &mut Option<DataPayload<'d, 's, D>>,
    provider: &P,
) -> Result<(), DateTimeFormatError>
where
    D: DataMarker<'s>,
    L: Clone + Into<LanguageIdentifier>,
    P: DataProvider<'d, 's, D> + ?Sized,
{
...
pub(super) struct TimeZoneFormat<'d, 's> {
    /// The pattern to format.
    pub(super) pattern: Pattern,
    /// The data that contains meta information about how to display content.
    pub(super) zone_formats: DataPayload<'d, 's, provider::time_zones::TimeZoneFormatsV1Marker>,
    /// The exemplar cities for time zones.
    pub(super) exemplar_cities:
        Option<DataPayload<'d, 's, provider::time_zones::ExemplarCitiesV1Marker>>,
    /// The generic long metazone names, e.g. Pacific Time
    pub(super) mz_generic_long:
        Option<DataPayload<'d, 's, provider::time_zones::MetaZoneGenericNamesLongV1Marker>>,
    /// The generic short metazone names, e.g. PT
    pub(super) mz_generic_short:
        Option<DataPayload<'d, 's, provider::time_zones::MetaZoneGenericNamesShortV1Marker>>,
    /// The specific long metazone names, e.g. Pacific Daylight Time
    pub(super) mz_specific_long:
        Option<DataPayload<'d, 's, provider::time_zones::MetaZoneSpecificNamesLongV1Marker>>,
    /// The specific short metazone names, e.g. Pacific Daylight Time
    pub(super) mz_specific_short:
        Option<DataPayload<'d, 's, provider::time_zones::MetaZoneSpecificNamesShortV1Marker>>,
}

impl<'d, 's> TimeZoneFormat<'d, 's> {
    ...
    #[allow(unused)]
    pub(super) fn format<'l: 's, T>(&'l self, value: &'l T) -> FormattedTimeZone<'l, T>
    where
        T: TimeZoneInput,
    {
        FormattedTimeZone {
